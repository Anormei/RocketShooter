...
import com.ar_co.androidgames.rocketshooter.framework.actions.MoveAction;
...
public class TestScreen2 extends Screen {
    ...
    public void onStart(){
        ...
        square = new Sprite(game, assets.getImage("graphics/gameplay/crown"));
        square.pos.x = 496.0f;
        square.pos.y = 916.0f;

        square1 = new Sprite(game, null, 88.0f, 80.0f);
        square1.pos.x = 496.0f;
        square1.pos.y = 916.0f;

        //sequence = new Sequence(new Sequence.Timestamp(5.0f, new MoveAction(square1, 0, 0, 10.0f, false)));

        sequence = new Sequence(){
            @Override
            public void init(){
                blink(square1, 0.3f, 1.0f, 5, 0f);
                moveTo(square1, 0, 0, 10.0f, 5.0f);
                fadeEW(square1, 0, 10.0f, 15.0f);
            }
        };
    }
    ...
    public void update(float deltaTime){
        ...
        square.pos.x -= (maxVel * (accel[0]) / 1.2f) * deltaTime;
        square.pos.y -= (maxVel * (accel[1]) / 1.2f) * deltaTime;

        if(square.pos.x > WORLD_WIDTH - square.width){
            square.pos.x = WORLD_WIDTH - square.width;
        }
        ...
        if(square.pos.y > WORLD_HEIGHT - square.height){
            square.pos.y = WORLD_HEIGHT - square.height;
        }
